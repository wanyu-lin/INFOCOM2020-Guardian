...
def gen(src, index, dim=-1, out=None, dim_size=None, fill_value=0):
    dim = range(src.dim())[dim]  # Get real dim value. 0

    # Automatically expand index tensor to the right dimensions.
    if index.dim() == 1:
        index_size = list(repeat(1, src.dim())) # [1, 1]
        index_size[dim] = src.size(dim) # [# of target nodes, 1]
        index = index.view(index_size).expand_as(src) # each row is the same #

    # Generate output tensor if not given.
    if out is None:
        out_size = list(src.size()) # [dim of src 2 #s]
        dim_size = maybe_dim_size(index, dim_size) # number of nodes
        out_size[dim] = dim_size # [# of nodes, same dim as node feature vector]
...
def scatter_add(src, index, dim=-1, out=None, dim_size=None, fill_value=0):
    ...
    # print(src)
    # print(dim)
    # out: 0 matrix of size of node feature matrix
    # dim = 0
    # index: one column = origional index, same size as src (start node)
    # src = target node matrix
